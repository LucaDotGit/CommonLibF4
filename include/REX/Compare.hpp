...
namespace REX
{
	...
	inline constexpr auto NUMERIC_LESS = static_cast<OrderType>(-1);
	inline constexpr auto NUMERIC_EQUAL = static_cast<OrderType>(0);
	inline constexpr auto NUMERIC_GREATER = static_cast<OrderType>(1);
	inline constexpr auto NUMERIC_UNORDERED = static_cast<OrderType>(std::numeric_limits<OrderType>::min());

	[[nodiscard]] constexpr OrderType GetNumericStrongOrdering(std::strong_ordering a_ordering) noexcept
	{
		...
		if (a_ordering == std::strong_ordering::greater) {
			return NUMERIC_GREATER;
		}

		return NUMERIC_EQUAL;
	}

	[[nodiscard]] constexpr OrderType GetNumericPartialOrdering(std::partial_ordering a_ordering) noexcept
	{
		...
		if (a_ordering == std::partial_ordering::unordered) {
			return NUMERIC_UNORDERED;
		}

		return NUMERIC_EQUAL;
	}

	[[nodiscard]] constexpr OrderType GetNumericWeakOrdering(std::weak_ordering a_ordering) noexcept
	{
	...
	[[nodiscard]] constexpr bool IsNullOrEmpty(const CharT* a_value) noexcept
	{
		return !a_value || a_value[0] == static_cast<CharT>(0);
	}
	...
	template <REX::win32_character CharT>
	[[nodiscard]] constexpr bool IsNullOrEmpty(std::basic_string_view<CharT> a_value) noexcept
	{
	...
	template <REX::win32_character CharT>
	[[nodiscard]] constexpr bool IsNullOrWhiteSpace(std::basic_string_view<CharT> a_value) noexcept
	{
		if (!a_value.data()) {
			return true;
		}

		return std::all_of(a_value.begin(), a_value.end(), [](auto a_char) -> bool {
	...
	template <REX::win32_character CharT>
	[[nodiscard]] constexpr bool IsNullTerminated(std::basic_string_view<CharT> a_value) noexcept
	{
		return a_value.data() && a_value.data()[a_value.size()] == static_cast<CharT>(0);
	}
	...
	{
		return a_lhs == a_rhs || ToLower(a_lhs) == ToLower(a_rhs);
	}

	[[nodiscard]] inline bool EqualsIgnoreCase(wchar_t a_lhs, wchar_t a_rhs) noexcept
	{
		return a_lhs == a_rhs || std::towlower(a_lhs) == std::towlower(a_rhs);
	}

	template <REX::win32_character CharT>
	[[nodiscard]] constexpr bool EqualsIgnoreCase(std::basic_string_view<CharT> a_lhs, std::basic_string_view<CharT> a_rhs) noexcept //
	{
		return std::equal(a_lhs.begin(), a_lhs.end(), a_rhs.begin(), a_rhs.end(),
			[](auto a_leftChar, auto a_rightChar) -> bool {
	...
	{
		const auto basicComparison = a_lhs <=> a_rhs;
	...
	template <REX::win32_character CharT>
	[[nodiscard]] constexpr auto CompareIgnoreCase(std::basic_string_view<CharT> a_lhs, std::basic_string_view<CharT> a_rhs) noexcept //
	{
		return std::lexicographical_compare_three_way(a_lhs.begin(), a_lhs.end(), a_rhs.begin(), a_rhs.end(),
			[](auto a_leftChar, auto a_rightChar) -> auto {
	...
	[[nodiscard]] constexpr bool Contains(std::basic_string_view<CharT> a_lhs, std::basic_string_view<CharT> a_rhs) noexcept
	{
	...
	template <REX::win32_character CharT>
	[[nodiscard]] constexpr bool ContainsIgnoreCase(std::basic_string_view<CharT> a_lhs, std::basic_string_view<CharT> a_rhs) noexcept
	{
		return std::ranges::contains_subrange(a_lhs, a_rhs,
			[](auto a_leftChar, auto a_rightChar) -> bool {
				return EqualsIgnoreCase(a_leftChar, a_rightChar);
	...
	template <class CharT>
	[[nodiscard]] constexpr bool StartsWith(std::basic_string_view<CharT> a_lhs, std::basic_string_view<CharT> a_rhs) noexcept
	{
	...
	template <REX::win32_character CharT>
	[[nodiscard]] constexpr bool StartsWithIgnoreCase(std::basic_string_view<CharT> a_lhs, std::basic_string_view<CharT> a_rhs) noexcept
	{
		if (a_rhs.size() > a_lhs.size()) {
			return false;
		}

		return std::equal(a_rhs.begin(), a_rhs.end(), a_lhs.begin(),
			[](auto a_leftChar, auto a_rightChar) -> bool {
				return EqualsIgnoreCase(a_leftChar, a_rightChar);
			});
	}
	...
	template <class CharT>
	[[nodiscard]] constexpr bool EndsWith(std::basic_string_view<CharT> a_lhs, std::basic_string_view<CharT> a_rhs) noexcept
	{
	...
	template <REX::win32_character CharT>
	[[nodiscard]] constexpr bool EndsWithIgnoreCase(std::basic_string_view<CharT> a_lhs, std::basic_string_view<CharT> a_rhs) noexcept
	{
		if (a_rhs.size() > a_lhs.size()) {
			return false;
		}

		return std::equal(a_rhs.rbegin(), a_rhs.rend(), a_lhs.rbegin(),
			[](auto a_leftChar, auto a_rightChar) -> bool {
				return EqualsIgnoreCase(a_leftChar, a_rightChar);
			});
	}
	...
	template <class CharT>
	[[nodiscard]] constexpr std::size_t Find(std::basic_string_view<CharT> a_lhs, std::basic_string_view<CharT> a_rhs, std::size_t a_offset = 0) noexcept
	{
	...
	template <REX::win32_character CharT>
	[[nodiscard]] constexpr std::size_t FindIgnoreCase(std::basic_string_view<CharT> a_lhs, std::basic_string_view<CharT> a_rhs, std::size_t a_offset = 0) noexcept
	{
		if (a_rhs.empty()) {
			return 0;
		}

		const auto it = std::search(a_lhs.begin() + a_offset, a_lhs.end(), a_rhs.begin(), a_rhs.end(),
			[](auto a_leftChar, auto a_rightChar) -> bool {
				return EqualsIgnoreCase(a_leftChar, a_rightChar);
			});
	...
	template <class CharT>
	[[nodiscard]] constexpr std::size_t FindLast(std::basic_string_view<CharT> a_lhs, std::basic_string_view<CharT> a_rhs, std::size_t a_offset = std::basic_string_view<CharT>::npos) noexcept
	{
	...
	template <REX::win32_character CharT>
	[[nodiscard]] constexpr std::size_t FindLastIgnoreCase(std::basic_string_view<CharT> a_lhs, std::basic_string_view<CharT> a_rhs, std::size_t a_offset = std::basic_string_view<CharT>::npos) noexcept
	{
		if (a_rhs.empty()) {
			return a_lhs.size();
		}

		const auto rit = std::search(a_lhs.rbegin() + (a_lhs.size() - a_offset - a_rhs.size()), a_lhs.rend(), a_rhs.rbegin(), a_rhs.rend(),
			[](auto a_leftChar, auto a_rightChar) -> bool {
				return EqualsIgnoreCase(a_leftChar, a_rightChar);
			});
	...
	public:
		template <class CharT>
		[[nodiscard]] constexpr bool operator()(const CharT& a_lhs, const CharT& a_rhs) const //
			noexcept(REX::nothrow_three_way_comparable<CharT> && std::is_nothrow_convertible_v<CharT, std::basic_string_view<typename CharT::value_type>>)
			requires(REX::three_way_comparable<CharT> && std::is_convertible_v<CharT, std::basic_string_view<typename CharT::value_type>>)
		{
			const auto leftView = static_cast<std::basic_string_view<typename CharT::value_type>>(a_lhs);
			const auto rightView = static_cast<std::basic_string_view<typename CharT::value_type>>(a_rhs);
