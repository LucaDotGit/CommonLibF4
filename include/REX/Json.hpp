...
{
	template <class T>
	bool SettingLoad(const std::any& a_data, const key_t& a_key, T& a_value, const T& a_defaultValue);

	template <class T>
	bool SettingSave(const std::any& a_data, const key_t& a_key, T&& a_value);
}
...
namespace REX::Json
{
	...
	public:
		...
		{
			if (!a_isMain) {
				return Impl::SettingLoad(a_data, _key, this->_value, this->_defaultValue);
			}

			if (!Impl::SettingLoad(a_data, _key, this->_defaultValue, this->_defaultValue)) {
				return false;
			}
		...
		{
			return Impl::SettingSave(a_data, _key, this->_value);
		}
...
namespace std
{
	...
	public:
		...
		[[nodiscard]] constexpr auto format(const REX::Json::Setting<T>& a_value, FormatContext& a_ctx) const
		{
