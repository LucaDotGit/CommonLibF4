...
namespace Scaleform::GFx::AS3
{
	...
	class __declspec(novtable) ScriptVM
		: public NewOverrideBase<329>
	{
	public:
		enum ErrorID : std::int32_t
		{
		...
		[[nodiscard]] CallFrame* GetCurrCallFrame()
		{
			if (callStack.size > 0) {
				auto index = callStack.size - 1;
				return std::addressof(callStack.pages[index >> 6][index & 0x3F]);
			}
		...
		std::array<std::byte, 0xE8 - 0x18> unk008;	   // 018 - TODO
		ArrayPagedCC<CallFrame, 6, 64, 329> callStack; // 0E8
		std::array<std::byte, 0x2D0 - 0x198> unk198;   // 198 - TODO
	};
	static_assert(sizeof(ScriptVM) == 0x2D0);
}
