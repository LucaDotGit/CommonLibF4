...
namespace Scaleform::GFx
{
	...
	public:
		...
		static_assert(sizeof(ImportVisitor) == 0x08);
		...
		static_assert(sizeof(ResourceVisitor) == 0x08);

		// add
		virtual std::uint32_t GetVersion() const = 0;																															  // 04
		virtual std::uint32_t GetLoadingFrame() const = 0;																														  // 05
		virtual REX::Float32 GetWidth() const = 0;																																  // 06
		virtual REX::Float32 GetHeight() const = 0;																																  // 07
		virtual std::uint32_t GetFrameCount() const = 0;																														  // 08
		virtual REX::Float32 GetFrameRate() const = 0;																															  // 09
		virtual Render::Rect<REX::Float32> GetFrameRect() const = 0;																											  // 0A
