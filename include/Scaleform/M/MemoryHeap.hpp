...
namespace Scaleform
{
	...
	public:
		...
		enum class RootHeapParameters : std::int32_t
		{
			kMinAlign = 1 << 4,
			kGranularity = 1 << 14,
			kReserve = 1 << 14,
		...
		public:
			...
			std::size_t minAlign{ 0x10 };									   // 08
			std::size_t granularity{ static_cast<std::size_t>(0x08) * 0x400 }; // 10
			std::size_t reserve{ static_cast<std::size_t>(0x08) * 0x400 };	   // 18
		...
		static_assert(sizeof(HeapVisitor) == 0x08);
		...
		static_assert(sizeof(LimitHandler) == 0x08);
		...
		public:
			...
			virtual void OnAlloc(const MemoryHeap* a_heap, std::size_t a_size, std::align_val_t a_align, std::uint32_t a_sid, const void* a_ptr) = 0; // 03
		...
		static_assert(sizeof(HeapTracer) == 0x08);
		...
		virtual void* Alloc(std::size_t a_size, std::align_val_t a_align, const AllocInfo* a_info = nullptr) = 0;								 // 0A
		...
		virtual void* AllocAutoHeap(const void* a_thisPtr, std::size_t a_size, std::align_val_t a_align, const AllocInfo* a_info = nullptr) = 0; // 0E
