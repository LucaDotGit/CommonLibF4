...
namespace RE
{
	...
	public:
		...
		enum class MemoryState : std::int32_t
		{
			kOk = 0,
			kOutOfMemory = 1
		};
		...
		static_assert(sizeof(ExtendedInterface) == 0x08);

		virtual ~hkMemoryAllocator(); // 00

		// add
		virtual void* BlockAlloc(std::int32_t a_numBytes);														// 01
		virtual void BlockFree(void* a_ptr, std::int32_t a_numBytes);											// 02
		...
		virtual void GetMemoryStatistics(MemoryStatistics& a_stats) const;										// 08
		virtual std::int32_t GetAllocatedSize(const void* a_obj, std::int32_t a_numBytes) const;				// 09
		...
		__declspec(noalias) void free(void* a_ptr, std::size_t a_size) noexcept;
	};
