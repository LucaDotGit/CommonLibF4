...
namespace RE
{
	...
	public:
		...
		// add
		virtual hkMemoryRouter* MainInit(const FrameInfo& a_info, REX::Enum<Flags, std::int32_t> a_flags) = 0;										 // 01
		virtual hkResult MainQuit(REX::Enum<Flags, std::int32_t> a_flags) = 0;																		 // 02
		...
		virtual hkResult GetHeapSoftLimit(std::int32_t a_numBytes);																					 // 05
		virtual std::int32_t GetHeapSoftLimit() const;																								 // 06
		virtual bool SolverCanAllocSingleBlock(std::int32_t a_numBytes);																			 // 07
		virtual bool HeapCanAllocTotal(std::int32_t a_numBytes);																					 // 08
		...
		virtual void GarbageCollectThread(hkMemoryRouter& a_router);																				 // 0C
		virtual void GarbageCollectShared();																										 // 0D
		virtual void GarbageCollect();																												 // 0E
		virtual void AdvanceFrame();																												 // 0F
		virtual hkDebugMemorySystem* GetDebugInterface();																							 // 10
		virtual hkResult GetMemorySnapshot(hkMemorySnapshot& a_snapshot) const;																		 // 11
		virtual hkResult GetAllocationCallStack(const void* a_ptr, std::uint64_t* a_callStack, std::int32_t& a_stackSize, std::size_t& a_allocSize); // 12
		virtual void SetHeapScrubValues(std::uint32_t a_allocValue, std::uint32_t a_freeValue);														 // 13
		virtual std::uint32_t IsOk() const;																											 // 14
	};
