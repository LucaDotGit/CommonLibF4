...
namespace RE
{
	...
	public:
		...
		virtual bool DoGetSpeakerBias(std::uint32_t a_srcChannels, std::uint32_t a_channel, const std::array<REX::Float32, 8>& a_levels) const = 0; // 03
		...
		virtual bool DoGetAudibility(REX::Float32 a_distance) const = 0;																			// 07
		...
		virtual REX::Float32 DoGetReverbSendLevel() const = 0;																						// 0A
	...
	static_assert(sizeof(BSISoundOutputModel) == 0x08);
}
