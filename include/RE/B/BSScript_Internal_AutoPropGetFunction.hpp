...
{
	class __declspec(novtable) AutoPropGetFunction
		: public IFunction // 00
	{
	public:
		inline static constexpr auto RTTI{ RTTI::BSScript__Internal__AutoPropGetFunction };
		inline static constexpr auto VTABLE{ VTABLE::BSScript__Internal__AutoPropGetFunction };

		~AutoPropGetFunction() override = default; // 00
		...
		std::uint32_t GetParamCount() const override;																											   // 05 - { return 0; }
		void GetParam(std::uint32_t a_param, BSFixedString& a_paramName, TypeInfo& a_paramType) const override;													   // 06
		std::uint32_t GetStackFrameSize() const override;																										   // 07 - { return 0; }
		...
		FunctionType GetFunctionType() const override;																											   // 0B - { return FunctionType::kPropertyGetter; }
	...
	static_assert(sizeof(AutoPropGetFunction) == 0x30);
}
