...
{
	class ActorValue
		: public BSTSingletonImplicit<ActorValue> // 00
	{
	public:
		enum class AVType : std::int32_t
		{
			kDerivedAttribute = 0,
			kAttribute = 1,
			kSkill = 2,
		...
		{
			using FuncType = decltype(&ActorValue::GetSingleton);
			static const auto FUNC = REL::Relocation<FuncType>{ ID::ActorValue::Singleton };
		...
		ActorValueInfo* attackConditionAlt01;										// 328
		ActorValueInfo* attackConditionAlt02;										// 330
		ActorValueInfo* attackConditionAlt03;										// 338
		...
		std::array<std::uint32_t, std::to_underlying(AVType::kTotal)> avTypeCounts; // 450
	};
