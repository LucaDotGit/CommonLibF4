...
namespace RE
{
	...
	public:
		...
		inline static constexpr auto MENU_DEPTH{ UI_DEPTH_PRIORITY::kExamineMenu };

		enum class INSPECT_MODE_STATE : std::int32_t
		{
			kModSlotSelect = 0,
			kSelectNewMod = 1,
		...
		{
			using FuncType = decltype(&ExamineMenu::GetSelectedIndex);
			static const auto FUNC = REL::Relocation<FuncType>{ ID::ExamineMenu::GetSelectedIndex };
		...
		REX::Enum<INSPECT_MODE_STATE, std::int32_t> inspectModeState;	  // 368
		...
		BSTArray<BSTPair<TESBoundObject*, std::uint32_t>> scrappingArray; // 420
		BSTArray<BSTPair<NiAVObject*, const BGSKeyword*>> slotObjects;	  // 440
		...
		REX::Float32 zoomDistance;										  // 55C
		REX::Float32 lastFrameDelta;									  // 560
		REX::Float32 menuCursorLeftPct;									  // 564
		REX::Float32 menuCursorRightPct;								  // 568
		REX::Float32 menuCursorTopPct;									  // 56C
		REX::Float32 menuCursorBottomPct;								  // 570
		BSTArray<BGSComponent*> queuedSoundArray;						  // 578
		std::array<std::byte, 0x598 - 0x590> attachPointSource3D;		  // 590 - TODO: `BSResource::RHandleType<BSResource::Entry<NiPointer<NiNode>, BSResource::EntryDBTraits<BSModelDB::DBTraits, BSResource::EntryDB<BSModelDB::DBTraits>>::CArgs>, BSResource::EntryDB<BSModelDB::DBTraits>>`
		BGSMod::Attachment::Mod* nullMod;								  // 598
		::Scaleform::GFx::Value requirementsList;						  // 5A0
		::Scaleform::GFx::Value itemSelectList;							  // 5C0
		BGSKeyword* keyword;											  // 5E0
		...
		std::array<char, 260> renameItemTo;								  // 5F1
		std::array<char, 260> renameItemCancelState;					  // 6F5
